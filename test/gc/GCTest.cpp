...
TEST(GC, AllocatorHostRootsRetainRelease) {
  ...
  Alloc.manualCollect(); // new object: gray -> black -> white
  EXPECT_GT(Alloc.getMemoryUsage(), 0u);
  Alloc.manualCollect(); // retained: re-grayed, survives the sweep
  EXPECT_GT(Alloc.getMemoryUsage(), 0u);

  Alloc.releaseRef(Ref);
  Alloc.manualCollect(); // unrooted now -> swept
...
TEST(GC, ExecutorRetainAndRelease) {
  ...
  Alloc.manualCollect(); // new -> white
  Alloc.manualCollect(); // retained by HostRoots -> survives
  EXPECT_GT(Alloc.getMemoryUsage(), Before);

  VM.getExecutor().releaseRef(Ref);
  Alloc.manualCollect(); // unrooted -> reclaimed
