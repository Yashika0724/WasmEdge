...
TEST(GC, gc) {
  ...
  // After dropping the 96-byte array and collecting, the only object ever
  // allocated so far is that array, so usage cannot exceed it (conservative
  // stack scanning may still keep it alive, hence the upper bound rather than
  // an exact 0).
  ...
  // The 224-byte array is still on the operand stack here (drop happens
  // afterwards), so it must remain retained.
