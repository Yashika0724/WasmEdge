...
// Path keys of shared libraries already processed, so repeated load() calls
// skip re-dlopen and duplicate registration. Guarded by Plugin::Mutex.
...
bool Plugin::loadFile(const std::filesystem::path &Path) noexcept {
  ...
  // Skip files that have already been processed in this process.
  const std::string FileKey = Path.u8string();
  if (!LoadedFiles.insert(FileKey).second) {
    return false;
  }
