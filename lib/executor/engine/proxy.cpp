...
Expect<RefVariant> Executor::proxyStructNew(Runtime::StackManager &StackMgr,
  ...
  Allocator.autoCollect(Allocator.getStack());
  if (Args == nullptr) {
...
Expect<uint32_t> Executor::proxyRefTest(Runtime::StackManager &StackMgr,
  ...
  if (!VT.isAbsHeapType()) {
    ...
    // `const ModuleInstance *`, which is all that is needed here, so read it
    // through CompositeBase instead of assuming the GC RawData layout (which
    // would misinterpret a typed funcref). The reference must not be nullptr
    // because null references carry the least abstract heap type.
    const auto *RefMod =
        Ref.getPtr<Runtime::Instance::CompositeBase>()->getModule();
    if (RefMod) {
