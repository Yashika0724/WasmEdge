...
inline Expect<uint64_t> parseUint(std::string_view Text) {
  // std::isdigit has undefined behavior for a negative char (non-ASCII byte),
  // so widen through unsigned char before classifying.
...
inline Expect<std::string> parseString(std::string_view Text,
  ...
  while (!Text.empty()) {
    ...
    if (Text.empty()) {
      // A trailing backslash with no escape char is an incomplete escape
      // sequence. The tree-sitter token regex never yields this, but reject it
      // so callers on unvalidated text do not silently get a truncated string.
    ...
    case '\'':
      ...
      break;
    case 'u': {
      // \u{...} must be a brace-delimited, non-empty run of hex digits naming a
      // code point <= U+10FFFF. Reject every malformed shape (missing brace,
      // empty payload, non-hex char, missing closer, out-of-range) rather than
      // silently decoding a partial or zero code point.
      if (Text.empty() || Text.front() != '{') {
        return Unexpect(ErrCode::Value::WatMalformedString);
      }
      Text = Text.substr(1); // consume '{'
