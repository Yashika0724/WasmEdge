...
TypeF<T> Executor::runAbsOp(Runtime::StackManager &StackMgr) const noexcept {
  ...
  // Clear the sign bit through a memcpy-based bit-cast to avoid the
  // strict-aliasing UB of reinterpret_cast'ing between the float and its
  // integer representation.
  if constexpr (sizeof(T) == 4) {
    uint32_t Tmp;
    std::memcpy(&Tmp, &Fp, sizeof(Tmp));
    Tmp &= UINT32_C(0x7fffffff);
    std::memcpy(&Fp, &Tmp, sizeof(Fp));
    StackMgr.emplaceTop<T>(static_cast<T &&>(Fp));
  } else {
    uint64_t Tmp;
    std::memcpy(&Tmp, &Fp, sizeof(Tmp));
    Tmp &= UINT64_C(0x7fffffffffffffff);
    std::memcpy(&Fp, &Tmp, sizeof(Fp));
    StackMgr.emplaceTop<T>(static_cast<T &&>(Fp));
  }
#else
